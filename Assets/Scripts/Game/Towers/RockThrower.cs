...
public class RockThrower : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		projectileShootFromPosition = transform.Find("rockThrowerShootFromPosition").position; //start point for projectile
	...
	private void Update()
	{
		...
		if (shootTimer <= 0f) //shoots projectile when timer is 0
		{
			...
			{
				RockProjectile.Create(projectileShootFromPosition, nearestEnemy, damageAmount);
			}
		}
