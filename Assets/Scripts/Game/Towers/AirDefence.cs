...
public class AirDefence : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		projectileShootFromPosition = transform.Find("airProjectileShootFromPosition").position; //start point for projectile
	...
	private void Update()
	{
		...
		if (shootTimer <= 0f) //shoots projectile when timer is 0
		{
			...
			{
				RocketProjectile.Create(projectileShootFromPosition, nearestEnemy, damageAmount);
	...
	void UpdateTarget ()
	{
		...
		foreach(GameObject enemy in enemies)
		{
			...
			if (distanceToEnemy < shortestDistance && enemy_.getIsAir() == true)
			{
