...
public class ArcherTower : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		projectileShootFromPosition = transform.Find("archerProjectileShootFromPosition").position; //start point for projectile
	...
	private void Update()
	{
		...
		if (shootTimer <= 0f) //shoots projectile when timer is 0
		{
			...
			{
				ArcherProjectile.Create(projectileShootFromPosition, nearestEnemy, damageAmount);
