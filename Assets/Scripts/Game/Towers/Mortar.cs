...
public class Mortar : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		projectileShootFromPosition = transform.Find("mortarProjectileShootFromPosition").position; //start point for projectile
	...
	private void Update()
	{
		...
		if (shootTimer <= 0f) //shoots projectile when timer is 0
		{
			...
			{
				BombProjectile.Create(projectileShootFromPosition, nearestEnemy, damageAmount);
			}
		}
